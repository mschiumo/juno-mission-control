...
export interface DailyRecapAiSummary {
  marketRecap: string;
  notableMovers: { symbol: string; move: string; reason: string }[];
  macroRecap: string[];
  earningsRecap: string[];
  newsHighlights: { headline: string; url: string }[];
  lookingAhead: string[];
  sentiment: 'bullish' | 'bearish' | 'neutral' | 'mixed';
}

interface DailyRecapEmailProps {
  date: string;
  indices: MarketItem[];
  stocks: MarketItem[];
  crypto: MarketItem[];
...
/* ------------------------------------------------------------------ */
/*  Design tokens — Dark Precision palette (shared with the briefing)  */
/* ------------------------------------------------------------------ */
...
const SECTION_ICONS = { close: '🔔', movers: '⚡', notable: '🎯', macro: '🏛️', earnings: '📊', news: '📰', ahead: '🔭' };
...
/* ------------------------------------------------------------------ */
/*  Day mover row (top gainers / losers of the session)                */
/* ------------------------------------------------------------------ */

function MoverRow({ mover }: { mover: DayMover }) {
  ...
  return (
    <Row style={{ ...tickerRow, backgroundColor: isUp ? 'rgba(0,200,150,0.03)' : 'rgba(255,61,87,0.03)' }}>
      ...
      <Column style={{ width: '42%', verticalAlign: 'middle' as const }}>
        <Link href={getTickerUrl(mover.symbol)} style={tickerName}>{mover.symbol}</Link>
      </Column>
      <Column style={{ width: '27%', textAlign: 'right' as const, verticalAlign: 'middle' as const, paddingRight: '12px' }}>
        <Text style={tickerPrice}>${mover.price.toFixed(2)}</Text>
      </Column>
      <Column style={{ width: '26%', textAlign: 'right' as const, verticalAlign: 'middle' as const, paddingRight: '10px' }}>
        <Text style={{
          ...changePill,
      ...
      {items.map((item, i) => (
        <Row key={i} style={eventRow}>
          ...
          <Column style={{ paddingLeft: '12px', verticalAlign: 'top' as const }}>
            <Text style={eventItem}>{item}</Text>
          </Column>
        </Row>
      ))}
    </>
...
export function DailyRecapEmail({ date, indices, stocks, crypto, topMovers, aiSummary }: DailyRecapEmailProps) {
  ...
  const hasTopMovers = !!topMovers && (topMovers.gainers.length > 0 || topMovers.losers.length > 0);

  return (
    <EmailLayout previewText={`Daily Market Recap ${date} — ${aiSummary.sentiment}`}>

      {/* Header — title + sentiment */}
      <Section style={card}>
        <Row>
          <Column style={{ verticalAlign: 'middle' as const }}>
            <Text style={dateText}>Daily Market Recap</Text>
            <Text style={dateSubtext}>{date} · After the close</Text>
          </Column>
      ...
      {/* How the day went */}
      <Section style={card}>
        <Text style={overviewText}>{aiSummary.marketRecap}</Text>
      </Section>

      {/* Closing Snapshot */}
      <Section style={card}>
        <SectionHeader icon={SECTION_ICONS.close} title="Closing Snapshot" />
      ...
      {hasTopMovers && (
        <Section style={card}>
          <SectionHeader icon={SECTION_ICONS.movers} title="Today's Top Movers" />
          {topMovers!.gainers.length > 0 && (
            <>
              <Text style={groupLabel}><span style={{ marginRight: '6px' }}>🟢</span>Top Gainers</Text>
              {topMovers!.gainers.map((m) => <MoverRow key={m.symbol} mover={m} />)}
            </>
          )}
          {topMovers!.losers.length > 0 && (
            <>
              <Text style={groupLabel}><span style={{ marginRight: '6px' }}>🔴</span>Top Losers</Text>
              {topMovers!.losers.map((m) => <MoverRow key={m.symbol} mover={m} />)}
            </>
          )}
        </Section>
      )}

      {/* Notable Movers — with the why */}
      {aiSummary.notableMovers.length > 0 && (
        <Section style={card}>
          <SectionHeader icon={SECTION_ICONS.notable} title="Notable Movers" />
          {aiSummary.notableMovers.map((mover, i) => {
            const isDown = mover.move.startsWith('-');
            return (
              <Section key={i} style={moverBlock}>
                <Row>
      ...
      {/* Macro & Economic Data */}
      {aiSummary.macroRecap.length > 0 && (
        <Section style={card}>
          <SectionHeader icon={SECTION_ICONS.macro} title="Macro & Economic Data" />
          <BulletList items={aiSummary.macroRecap} />
        </Section>
      )}

      {/* Earnings */}
      {aiSummary.earningsRecap.length > 0 && (
        <Section style={card}>
          <SectionHeader icon={SECTION_ICONS.earnings} title="Earnings" />
          <BulletList items={aiSummary.earningsRecap} />
        </Section>
      )}
      ...
      {aiSummary.newsHighlights.length > 0 && (
        <Section style={card}>
          <SectionHeader icon={SECTION_ICONS.news} title="News That Moved Markets" />
          {aiSummary.newsHighlights.map((item, i) => (
            <Row key={i} style={newsRow}>
              ...
              <Column style={{ verticalAlign: 'top' as const }}>
                <Text style={newsItem}>
                  {item.url
                    ? <Link href={item.url} style={newsLink}>{item.headline}</Link>
                    : item.headline}
                </Text>
              </Column>
            </Row>
      ...
      {/* Looking Ahead */}
      {aiSummary.lookingAhead.length > 0 && (
        <Section style={card}>
          <SectionHeader icon={SECTION_ICONS.ahead} title="Looking Ahead" />
          <BulletList items={aiSummary.lookingAhead} />
        </Section>
      )}
...
const moverBlock: React.CSSProperties = {
  padding: '11px 0',
  borderBottom: `1px solid ${P.borderSubtle}`,
};
