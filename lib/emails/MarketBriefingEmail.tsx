...
function TickerCopyBox({ stocks, label, color, icon }: {
  stocks: GapStock[]; label: string; color: string; icon: string;
}) {
    ...
    <EmailLayout previewText={`Market Briefing ${date} — ${aiSummary.sentiment}`}>
      ...
      {gapData && (gapData.gainers.length > 0 || gapData.losers.length > 0) && (
        <Section style={card}>
          ...
          <TickerCopyBox stocks={gapData.gainers} label="Gapping Up" color={P.positive} icon="🟢" />
          <TickerCopyBox stocks={gapData.losers} label="Gapping Down" color={P.negative} icon="🔴" />
        </Section>
      )}
