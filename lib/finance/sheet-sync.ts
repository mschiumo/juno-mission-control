...
import { DebtAccount, DebtType } from './types';
...
const TYPE_KEYWORDS: Array<[RegExp, DebtType]> = [
  [/credit/i, 'credit-card'],
  [/auto|car/i, 'auto-loan'],
  [/student/i, 'student-loan'],
  [/personal/i, 'personal-loan'],
  [/mortgage|home/i, 'mortgage'],
];
...
export function parseSheetCsv(text: string): SheetParseResult | string {
  ...
  rows.slice(1).forEach((row, i) => {
    ...
    const type = TYPE_KEYWORDS.find(([re]) => re.test(rawType))?.[1] ?? 'other';
  ...
  if (parsed.length === 0) return 'No valid account rows found in the sheet';
