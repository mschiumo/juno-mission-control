/**
 * Shared daily motivational quote — used by /api/daily-quote (dashboard
 * banner) and the morning market briefing cron so both surfaces show the
...
let cache: DailyQuote | null = null;
...
export async function getDailyQuote(): Promise<DailyQuoteResult> {
  const today = getTodayKey();

  // Return cached quote if it's still for today
  if (cache && cache.date === today) {
    return { data: cache, cached: true, fallback: false };
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(data) && data.length > 0 && data[0].q && data[0].a) {
        const fresh: DailyQuote = {
          quote: data[0].q,
          author: data[0].a,
          date: today,
        };
        cache = fresh;
        return { data: fresh, cached: false, fallback: false };
      }
    }
  } catch {
  ...
  return { data: result, cached: false, fallback: true };
}
