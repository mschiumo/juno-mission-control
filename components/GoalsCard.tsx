...
function SortableGoalCard({
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          {!isMobileView && (
            <button
              {...attributes}
              {...listeners}
              className="flex-shrink-0 text-[#737373] hover:text-[#F97316] transition-colors p-1 -ml-1 cursor-grab active:cursor-grabbing"
          ...
          <button
            ...
            className="flex-shrink-0 text-[#8b949e] hover:text-[#F97316] transition-colors p-1"
          >
            {isSelected ? (
  ...
  return (
    <div
      ...
      className={`group p-4 rounded-xl border cursor-pointer transition-all hover:shadow-lg ${phaseColors[phase]} border-opacity-30 ${isCollaborative ? 'ring-1 ring-purple-500/30' : ''} ${isDragging ? 'ring-2 ring-[#F97316]' : ''}`}
    >
      {cardContent}
    </div>
  );
...
// Mobile Goal Card Component (without drag functionality)
...
function MobileGoalCard({
  ...
  return (
    ...
    >
      ...
      {goal.notes && (
        <p className="text-xs text-[#737373] mt-2 line-clamp-2 italic">{goal.notes}</p>
      )}
...
export default function GoalsCard() {
  ...
  // Reorder goals within the same phase
  const handleReorderGoals = async (phase: Phase, oldIndex: number, newIndex: number) => {
    ...
    updatedGoals[activeCategory] = updatedGoals[activeCategory].map(goal => {
      ...
      if (newOrderIndex !== -1 && goal.phase === phase) {
        return { ...goal, order: newOrderIndex };
      }
      return goal;
    });
    setGoals(updatedGoals);
    ...
    try {
      ...
      if (!response.ok) {
        // Revert on error
        fetchGoals();
        showNotification('Failed to reorder goals', 'error');
      }
    } catch (error) {
  ...
  const getGoalsByPhase = (category: Category, phase: Phase) => {
    ...
    // Sort by order field, falling back to index if no order set
    return filtered.sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  };

  // Search filter function - searches across all goal fields
  const filterGoalsBySearch = (goalsList: Goal[]): Goal[] => {
    ...
    return goalsList.filter(goal => {
      ...
      const actionItemsMatch = goal.actionItems?.some(item => 
        item.text?.toLowerCase().includes(query)
      );
      const sourceMatch = goal.source?.toLowerCase().includes(query);
      
      return titleMatch || notesMatch || actionItemsMatch || sourceMatch;
    });
  };

  // Get filtered goals for current category
  const getFilteredGoalsByPhase = (category: Category, phase: Phase): Goal[] => {
    const phaseGoals = getGoalsByPhase(category, phase);
    return filterGoalsBySearch(phaseGoals);
  };

  // Get all goals matching search (for showing results count)
  const getAllMatchingGoals = (): Goal[] => {
    ...
    return filterGoalsBySearch(allGoals);
  };
        ...
        <div className="mb-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full px-10 py-2.5 bg-[#0F0F0F] border border-[#262626] rounded-xl text-white text-sm placeholder-[#737373] focus:outline-none focus:border-[#F97316] transition-colors"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-[#262626] rounded-lg transition-colors"
              >
          ...
          {searchQuery.trim() && (
            <div className="flex items-center justify-between mt-2 px-1">
              <span className="text-xs text-[#737373]">
                {getAllMatchingGoals().length} result{getAllMatchingGoals().length !== 1 ? 's' : ''}
              </span>
              <button
                onClick={() => setSearchQuery('')}
            ...
            return (
              <div key={phase} className="bg-[#0F0F0F] rounded-xl p-4 border border-[#262626]">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${isFiltered ? 'bg-[#F97316]/20 text-[#F97316]' : 'bg-[#1a1a1a] text-[#737373]'}`}>
                ...
                <div className="space-y-2">
                  ...
                  {phaseGoals.length === 0 && (
                    <div className="text-center py-4 text-[#737373] text-xs">
                      {searchQuery.trim() ? 'No matching goals' : 'No goals'}
                    </div>
                  )}
                </div>
              </div>
    ...
    <div className="card">
      ...
      <div className="mb-6">
        ...
        {searchQuery.trim() && (
          <div className="flex items-center justify-between mt-2">
            <span className="text-xs text-[#737373]">
              {(() => {
                const allMatching = getAllMatchingGoals();
                const personalCount = allMatching.filter(g => g.category !== 'collaborative').length;
                const collabCount = allMatching.filter(g => g.category === 'collaborative').length;
                return `${allMatching.length} result${allMatching.length !== 1 ? 's' : ''} ${personalCount > 0 && collabCount > 0 ? `(${personalCount} personal, ${collabCount} collaborative)` : ''}`;
              })()}
            </span>
            <button
              onClick={() => setSearchQuery('')}
              className="text-xs text-[#F97316] hover:text-[#ff8c5a] transition-colors"
            >
              Clear
            </button>
          </div>
      ...
      <DndContext
        ...
        onDragEnd={(event: DragEndEvent) => {
          ...
          // Find which phase the dragged goal is in
          const activeGoal = goals[activeCategory]?.find(g => g.id === activeId);
          if (!activeGoal) return;
          
          // Check if dropped in a different phase column (phase droppable IDs are phase names)
          ...
          if (activeId !== overId) {
            ...
            if (oldIndex !== -1 && newIndex !== -1) {
              handleReorderGoals(activeGoal.phase, oldIndex, newIndex);
            }
          }
        }}
      >
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full ${isFiltered ? 'bg-[#F97316]/20 text-[#F97316]' : 'bg-[#1a1a1a] text-[#737373]'}`}>
                    {isFiltered ? `${phaseGoals.length}/${totalPhaseGoals.length}` : phaseGoals.length}
                  </span>
                </div>

                <SortableContext
                  items={phaseGoals.map(g => g.id)}
                ...
                >
                  <div className="space-y-2 min-h-[100px]">
                    ...
                    {phaseGoals.length === 0 && (
                      <div className="text-center py-8 text-[#737373] text-sm border-2 border-dashed border-[#262626] rounded-xl">
                        {searchQuery.trim() ? 'No matching goals' : 'Drop goals here'}
                      </div>
                    )}
                  </div>
                </SortableContext>
        ...
        <DragOverlay>
          {activeGoal ? (
            <div className={`p-4 rounded-xl border-2 border-[#F97316] shadow-2xl bg-[#1a1a1a] ${phaseColors[activeGoal.phase]}`}>
              <div className="flex items-center gap-2">
                <GripVertical className="w-4 h-4 text-[#F97316]" />
                <p className="text-sm text-white truncate">{activeGoal.title}</p>
              </div>
            </div>
          ) : null}
        </DragOverlay>
