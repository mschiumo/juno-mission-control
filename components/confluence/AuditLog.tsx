  ...
  return (
    <div className="card">
      <h3 className="text-sm font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>
        Audit Log
        <span className="ml-2 text-[11px] font-normal" style={{ color: 'var(--text-tertiary)' }}>immutable · newest first</span>
      </h3>

      {events.length === 0 ? (
        <p className="text-[13px] py-6 text-center" style={{ color: 'var(--text-tertiary)' }}>No events yet.</p>
      ) : (
          ...
          {events.map((e) => {
            ...
            return (
              <li key={e.id} className="flex gap-3 pb-3.5">
                <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: 'var(--surface-2)', border: '1px solid var(--border-default)' }}>
                  <Icon className="w-3.5 h-3.5" style={{ color }} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline gap-2 flex-wrap">
                    <span className="text-[10px] uppercase tracking-wide font-semibold" style={{ color }}>{label}</span>
                    <span className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>{new Date(e.occurredAt).toLocaleString()}</span>
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ background: 'var(--surface-2)', color: 'var(--text-tertiary)' }}>{e.eventType}</span>
                  </div>
                  {e.note && <p className="text-[13px] mt-0.5" style={{ color: 'var(--text-secondary)' }}>{e.note}</p>}
                </div>
              </li>
            );
          })}
