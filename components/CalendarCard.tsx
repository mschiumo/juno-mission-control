...
const SETUP_STEPS = [
  {
    ...
    description: (
      <>
        ...
        and click the <span className="text-white">gear icon ⚙</span> in the top-right corner, then select{' '}
  ...
  },
  {
    ...
    description: (
      <>
        ...
        <span className="text-white">&ldquo;My calendars&rdquo;</span> and click on its name to open its settings.
      </>
    ),
  },
  {
    ...
    description: (
      <>
        Scroll down to <span className="text-white">&ldquo;Access permissions for events&rdquo;</span> and check{' '}
        <span className="text-white">&ldquo;Make available to public&rdquo;</span>. Click OK to confirm.
        This is required for the iCal feed to work.
      </>
    ),
  },
  {
    ...
    description: (
      <>
        Scroll further down to <span className="text-white">&ldquo;Integrate calendar&rdquo;</span> and copy the{' '}
        <span className="text-white">&ldquo;Public address in iCal format&rdquo;</span> URL.
        It starts with <span className="text-[#ff6b35]">https://calendar.google.com/calendar/ical/...</span>
      </>
    ),
  },
];
  ...
  return (
    <div className="bg-[#161b22] border border-[#30363d] rounded-xl flex flex-col h-full relative overflow-hidden">
      ...
      {showSetupModal && (
        ...
        >
          <div
            className="bg-[#161b22] border border-[#30363d] rounded-xl shadow-2xl w-full max-w-sm flex flex-col"
            onClick={e => e.stopPropagation()}
          >
            {/* Modal header */}
            <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[#30363d]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-white">Connect Google Calendar</span>
              </div>
              <button onClick={() => setShowSetupModal(false)} className="p-1 hover:bg-[#30363d] rounded transition-colors">
                <X className="w-4 h-4 text-[#8b949e]" />
              </button>
            </div>

            {/* Slide content */}
            <div className="px-5 py-6 flex-1 min-h-[220px] flex flex-col justify-center">
              {setupStep < SETUP_STEPS.length ? (
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="p-4 bg-[#ff6b35]/10 rounded-full">
                        <StepIcon className="w-7 h-7 text-[#ff6b35]" />
                      </div>
                    );
                  })()}
                  <div className="space-y-2">
                    <p className="text-[10px] font-semibold text-[#ff6b35] uppercase tracking-wider">
                      Step {setupStep + 1} of {totalSteps}
                    </p>
                    <h3 className="text-sm font-semibold text-white">{SETUP_STEPS[setupStep].title}</h3>
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col items-center text-center gap-2 mb-1">
                    <div className="p-4 bg-[#ff6b35]/10 rounded-full">
                      <Link className="w-7 h-7 text-[#ff6b35]" />
                    </div>
                    <p className="text-[10px] font-semibold text-[#ff6b35] uppercase tracking-wider">
                      Step {totalSteps} of {totalSteps}
                    </p>
                    <h3 className="text-sm font-semibold text-white">Paste Your iCal URL</h3>
                  </div>
                  <input
                    type="url"
                    value={icalInput}
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 bg-[#ff6b35] hover:bg-[#ff6b35]/90 disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-medium rounded-lg px-3 py-2.5 transition-colors"
                  >
